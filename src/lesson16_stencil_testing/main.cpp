...
int main() {
    ...
    Program outlineProgram(readFile("basic.vertex.glsl"), readFile("single_color.fragment.glsl"));
    ...
    float planeVertices[] = {
            // positions
            // texture Coords (note we set these higher than 1
    ...
    while (!glfwWindowShouldClose(window)) {
...

        {
            ...
            glBindVertexArray(planeVAO);
            floorTexture.bind();
            basicProgram.setUniform("model", glm::mat4(1.0f));
...

        {
            ...
            glm::mat4 projection = glm::perspective(glm::radians(camera.zoom()), (float) width / (float) height, 0.1f,
                                                    100.0f);
            outlineProgram.setUniform("view", view);
            outlineProgram.setUniform("projection", projection);
            ...
            outlineProgram.setUniform("model", model);
            glDrawArrays(GL_TRIANGLES, 0, 36);
